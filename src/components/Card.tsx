...
const Card: React.FC<CardProps> = ({
    ...
    return (
        <div
            className={`h-[80px] rounded-lg cursor-pointer hover:shadow-md hover:shadow-blue-500/50 hover:scale-105 transition-transform duration-300 ease-out border border-transparent hover:border-blue-500`}
            onClick={() => {
                router.push(
                    `/?name=${encodeURIComponent(
                        scene.name
                    )}&rec_no=${encodeURIComponent(rec_no.toString())}&benchmark=${encodeURIComponent(scene.benchmark)}`
                );
            }}
        >
            <div className="bg-offwhite shadow-lg rounded-lg h-[80px] flex flex-col justify-center items-center">
                <div className="flex flex-row h-[80px] w-full gap-2 justify-start">
                    <div className="flex flex-row justify-center items-center min-w-[50px] rounded-tl-lg rounded-bl-lg border-r-2 gap-0.5 border-r-black">
                        <span
                            className={`text-xl font-medium ${isSelected ? "text-blue-500" : ""
                                }`}
                        >
                            #
                        </span>
